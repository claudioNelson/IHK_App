...
**Python-Kurs (gestartet 21.08.2026, gleiche Engine):** `lib/data/kurse/python_kurs.dart`, Lektion 1-7 geschrieben (print · Variablen · Rechnen/Text · input/f-Strings · if/elif/else · Listen · for). Kein Live-Python in der App: Aufgabentypen sind Luecke/Reihenfolge/Fehler/Auswahl, Ausgabe-Vorhersage ersetzt das Ausfuehren. Roter Faden wieder Nordwind GmbH. Kachel im Lern-Hub (Tag PY, gelb). `Kurs.lektionenGeplant = 14`: Badges (kurs_python_start 🐍 / haelfte 🔥 / meister 👑, in DB) rechnen gegen die geplanten 14, nicht die vorhandenen Lektionen. Engine-Fixes dabei: alleinstehende Ueberschrift landet auf der Aufgabenseite (statt leerer Seite), Seiten vertikal zentriert + Akzentbalken + Icon auf Textseiten, ListTile-Warnung (Tipp-ExpansionTile in Material gewrappt). OFFEN: Lektion 8-14 (while · Funktionen 1+2 · Dictionaries · Strings · try/except · Abschluss/Pseudocode), premium-Flag ab Lektion 8 setzen.
