...
- **IHK-Pruefungen jetzt in der DB** (Migration 20260901100000_ihk_exams.sql): fuenf Zeilen in `exams` mit typ 'ihk' und neuer Spalte `slug` ('ae-1' ... 'si-2'), Ids 14 bis 18. Die alten Zeilen 1 bis 13 sind Zertifikats-Uebungen (typ 'uebung') und tauchen im Profil unter Pruefungen NICHT mehr auf. **OFFEN (naechstes App-Paket):** Die App speichert Simulations-Ergebnisse noch NICHT in `user_exam_attempts` (status-Werte: in_progress/submitted/graded). Beim Einbau gleich `checkExamBadges` aufrufen (hat bis heute keinen Aufrufer). Testdaten fuer claudionelson1989@gmail.com (4 Zertifikate, 3 Pruefungen) liegen in der DB, spaeter mit Delete auf user_id aufraeumen.
