...
- ⬜ **Web-Redesign, Teil 2 (offen):** Auth (login, signup, reset-confirm, update-password, auth/callback), Konto (profil, upgrade mit veralteten Inhalten „937 Fragen"/„33 Levels", account-loeschung), Rechtsseiten-Optik (Tailwind-Grau, ohne Shell/Dunkelmodus), Fragetyp-Komponenten der Pruefung (TableInput, DecisionMatrix, CodeCorrection, FillBlanks, DiagramTool, ImageLightbox fest hell), `not-found.tsx`, Metadata-Durchgang, „4 von 5 waehlen" auswerten. **Mockup fuer Auth + Konto + Upgrade liegt fertig vor** (`/home/claude/auth-design/` in der Cowork-Sitzung vom 23.09.; Entscheidungen offen: `premium_source`-Spalte fuer Web/Google/Apple im Profil, Passwortregel 8 statt 6 Zeichen, Abmelden nur lokal `signOut({scope:"local"})`, Fortschritts-Abschnitt im Profil).
